...
import { Filter, ChevronDown, ChevronUp, Search, ChevronLeft, ChevronRight, PackageSearch } from 'lucide-react';
      ...
      <div className="container-custom section-padding">
        <div className="flex flex-col lg:flex-row gap-6 lg:gap-8">
          ...
          <div className="lg:w-64 flex-shrink-0">
            ...
            <div
              id="filter-sidebar"
              className="bg-white rounded-lg border border-sand/50 p-5 space-y-6"
              ...
              <div>
                ...
                <form onSubmit={handleSearch} className="flex gap-2">
                  <input
                    ...
                    className="flex-1 px-3 py-2 border border-sand rounded text-sm bg-cream focus:outline-none focus:border-terracotta"
                  />
                  <button type="submit" className="px-3 py-2 bg-terracotta text-white rounded text-sm hover:bg-terracotta-dark transition-colors flex items-center justify-center">
                    <Search size={16} />
                  </button>
                </form>
              </div>
                ...
                <div className="space-y-1">
                  ...
                  {categories.map(cat => (
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 rounded text-sm transition-colors ${
                        selectedCategory === cat.slug ? 'bg-terracotta/10 text-terracotta font-medium' : 'text-taupe hover:text-charcoal hover:bg-warm-beige'
                      }`}
                    >
                ...
                <div className="space-y-1">
                  {[
                    ...
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 rounded text-sm transition-colors ${
                        priceRange === range.value ? 'bg-terracotta/10 text-terracotta font-medium' : 'text-taupe hover:text-charcoal hover:bg-warm-beige'
                      }`}
                    >
                      {range.label}
                    </button>
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-6 pb-4 border-b border-sand/50">
              ...
              <div className="flex items-center gap-2">
                <label className="text-sm text-taupe">Sắp xếp:</label>
                <select
                  value={sortBy}
                  onChange={(e) => setSortBy(e.target.value)}
                  className="px-3 py-1.5 border border-sand rounded text-sm bg-white text-charcoal focus:outline-none focus:border-terracotta"
                >
            ...
            {paginatedProducts.length > 0 ? (
              <div className="product-grid grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 md:gap-6">
